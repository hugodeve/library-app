            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <nav class="text-sm" aria-label="Breadcrumb">
                        <ol class="list-none p-0 inline-flex">
                            ...
                            <li class="flex items-center">
                                <a href="{{ route('integrations.edit', $integration) }}" class="font-semibold text-gray-700">
                    ...
                    <section>
                        <form action="{{ route('integrations.update', $integration) }}" method="POST">
                            ...
                            <div>
                                ...
                                <div class="mt-2.5">
                                <input type="text" name="nome_integracao"  id="nome_integracao"  value="{{ $integration->nome_integracao }}" placeholder="Nome da integração" autocomplete="given-name" class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6" required>
                            </div>
                            </div>
                            <div>
                                <label for="pessoa_id" class="block text-sm font-semibold leading-6 text-gray-900">Pessoas</label>
                                <select id="pessoa_id" name="pessoa_id" class="h-full rounded-md border-0 bg-transparent bg-none py-0 pl py-0 pl-4 pr-9 text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm" required>
                                    <option value="">Escolha uma Pessoa</option>
                                    @foreach ($pessoas as $pessoa)
                                        <option value="{{ $pessoa->id }}" {{ $pessoa->id == $selectedPessoaId ? 'selected' : '' }}>{{ $pessoa->nome }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select class="h-full rounded-md border-0 bg-transparent bg-none py-0 pl-4 pr-9 text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm" name="unidade_id"  id="unidade_id" required>
                                    ...
                                    @foreach ($unidades as $unidade)
                                        <option value="{{ $unidade->id }}" {{ $unidade->id == $integration->unidade_id ? 'selected' : '' }}>{{ $unidade->nome }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div >
                                ...
                                <select class="h-full rounded-md border-0 bg-transparent bg-none py-0 pl-4 pr-9 text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm" name="status"  id="status" required>
                                    <option value="{{ $integration->status }}">{{ $integration->status }}</option>
